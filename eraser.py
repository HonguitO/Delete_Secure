...
def crear_dir():
    ...
    #controla si el directorio ya fue creada
    if os.path.exists(direc):
        print "EL DIRECTORIO YA FUE CREADO...."
        sleep(2)
        crear_dir
    else:
        #en caso que no, crea un directorio
        print "CREADO DIRECTORIO...."
        sleep(2)
        os.mkdir(direc)
        #controla si se creo el directorio
    if os.path.exists(direc):
...
def liberar_men():
    ...
    ejec.stdout.close()
    ##controla si se instalo smen, si se instalo libera la memoria
    if not no_install:
        print cmd
        ...
        ##en caso contrario instala smem
        print "SMEM SE INSTALARA EN TU SISTEMA..... :D"
        sleep(3)
        os.system("sudo apt-get install smem -y")
        os.system("clear")
        menu_principal()
